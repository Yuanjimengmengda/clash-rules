...
function Update-SecoClientPac {
  ...
  if (-not (Test-Path -LiteralPath $Path)) {
    Write-Host "PAC not found yet: $Path"
    return $false
  }
  ...
  if ($updated -ne $content) {
    ...
    Write-Host "Patched PAC: $Path"
    return $true
  }

  Write-Host "PAC already patched: $Path"
  return $false
}
...
function Update-WindowsProxyBypass {
  ...
  Write-Host "Updated Windows proxy bypass list."
}

$pacChanged = Update-SecoClientPac -Path $PacPath -DomainList $Domains
Update-WindowsProxyBypass -DomainList $Domains
